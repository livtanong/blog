...
and learning process, it may feel a bit hand-holdy and may appear to
over-explain. I want this article to be as accessible as possible, so please
bear with me.
...
| xact        | A group of transactions tied by a date and some description, usually just the payee. What a lot of peple, myself included, would intuitively think of as the "transaction"              | The whole thing above          |
...
Looks like we aren't doing anything fancy. We can extend this algorithm to get
the date from the same ~xact~ line. As for the amount, since only one of the
amounts should matter in the entire posting,
