...
of the Airtable base it wants to talk to. In my company, for instance, we don't
require time--only date. The fields are all different as well. I could factor
this out to accept some function that allows one to build out the ~"fields"~
data structure, but guess what, that's already the meat of our implementation.
